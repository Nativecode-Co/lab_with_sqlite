...
function visitDetail(hash, name) {
  HASH = hash;
  ...
  const show_visit_button = $("#show_visit_button");
  const invoice_button = $("#invoice_button");
  const print_barcode = $("#print_barcode");
  ...
  print_barcode.attr("onclick", `printBarcode('${hash}', '${name}')`);
}
...
function showVisit(hash) {
  ...
  if (!visit) {
    niceSwal("error", "top-end", "حدث خطأ ما");
    return;
  }
...
function showInvoice(hash) {
  ...
  if (!visitPackages || !visit) {
    niceSwal(
      "error",
      "top-end",
      "حدث خطأ ما حدث خطأ أثناء جلب البيانات الخاصة بالفاتورة"
    );
  }
  const invoice = `
...
function showResult(data) {
  ...
  const defaultHeight = (invoice.center ?? 1200) - 270;
  ...
  Swal.fire({
    ...
    <form id="barcode-form" class="text-left">
      <div class="form-group
        ...
        <input type="number" class="form-control mt-3" id="barcode_height" placeholder="طول الباركود" value="${
          invoices?.barcode_height ?? 40
        }">
      </div>
      <div class="form-group">
        <label for="barcode_width">عرض الباركود (mm)</label>
        <input type="number" class="form-control mt-3" id="barcode_width" placeholder="عرض الباركود" value="${
