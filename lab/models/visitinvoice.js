...
class Visit extends Factory {
    ...
    this.dataTable = setServerTable(
      ...
      [
        ...
        },
        {
          ...
          render: (data, type, row) => {
            return `
            <a href="#" class="w-100 d-block" onclick="visitDetail('${row.hash}', '${row.name}');fireSwalWithoutConfirm(showVisit, '${row.hash}', '${row.name}')">
              ${row.visit_date}
            </a>`;
          },
        },
  ...
  saveUpdateItem(hash) {
    ...
    visitDetail(hash, data.name);
    showVisit(hash, data.name);
    newPatientElement.checked = true;
    changePatientTag();
  }
