...
class Package_model extends CI_Model
{
    ...
    function getOffersForLAb($labID, $start, $length, $search)
    {
        // like
        $this->db->select(
    ...
    {
        $packages = $this->db->query('SELECT hash,test_name,option_test->"$.component[0].reference[*].kit" as kit,option_test->"$.component[0].reference[*].unit" as unit FROM lab_test group by test_name');
    ...
    function getKitsByTestName($testName)
    {
        $kits = $this->db->query(
            "SELECT  
                CASE
    ...
    function getUnitsByTestName($testName)
    {
        $units = $this->db->query(
            "SELECT  
            CASE
